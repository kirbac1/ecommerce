...
@section('page.title', trans('messages.Products'))
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <th>{{ trans('messages.Quantity Per Pack') }}</th>
                    <th>{{ trans('messages.Net Price') }}</th>
                    <th>{{ trans('messages.Tax') }}</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="(key, val) in localizedObjects" class="clickableRow">
                    <td>
                        <div class="checkbox m-b-15">
                            <label>
                                <input type="checkbox" v-model="val.selected" @click="selectAllStatus=false">
                                <i class="input-helper"></i>
                            </label>
                        </div>
                    </td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.manufacturer }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.name }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.barcode }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.sku }}</td>
                    <!--<td v-on:click.prevent="gotoDetails(val.id)">@{{ val.category.name || '' }}</td>-->
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.visible }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.qtyPerPack }} @{{ val.measureunit }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">&euro;@{{ val.priceEach }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.taxPercent }}%</td>
                </tr>
                </tbody>
            </table>
            <!-- Pagination -->
        ...
        var vue = new Vue({
            ...
            computed: {
                ...
                localizedObjects: function() {
                    ...
                    this.objects.forEach(function(object) {
                        if (object.visible) {
                            object.visible = '{!! trans('messages.Visible') !!}';
                        } else {
                            object.visible = '{!! trans('messages.Hidden') !!}';
                        }
            ...
            methods: {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function(object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/products/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/products/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
