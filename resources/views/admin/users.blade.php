...
@section('page.title', trans('messages.Users'))
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <th>{{ trans('messages.E-Mail') }}</th>
                    <th>{{ trans('messages.Enabled') }}</th>
                ...
                <tr v-for="(key, val) in localizedObjects | filterBy searchFilter" class="clickableRow">
                    ...
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.name }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.surname }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.email }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.enabled }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.type }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.language }}</td>
                </tr>
                </tbody>
            </table>
            <!-- Pagination -->
        ...
        var vue = new Vue({
            ...
            computed: {
                ...
                localizedObjects: function() {
                    ...
                    this.objects.forEach(function(object) {
                        ...
                        if (object.enabled == '1') {
                            object.enabled = '{!! trans('messages.prop.boolenabled') !!}';
                        } else if (object.enabled == '0') {
                            object.enabled = '{!! trans('messages.prop.booldisabled') !!}';
                        }

                        // Translate language
                        if (object.language == 'fi') {
                            object.language = '{!! trans('messages.prop.languageFI') !!}';
                        } else if (object.language == 'en') {
                            object.language = '{!! trans('messages.prop.languageEN') !!}'
                        } else if (object.language == 'it') {
                            object.language = '{!! trans('messages.prop.languageIT') !!}';
                        }

                        // Translate roles
                        if (object.type == 'admin') {
                            object.type = '{!! trans('messages.prop.admin') !!}';
                        } else if (object.type == 'worker') {
                            {{--object.type = '{!! trans('messages.prop.worker') !!}'--}}
                        } else if (object.type == 'webuser') {
                            {{--object.type = '{!! trans('messages.prop.webuser') !!}'--}}
                        } else if (object.type == 'cashier') {
                            object.type = '{!! trans('messages.prop.cashier') !!}'
                        } else if (object.type == 'customer') {
                            {{--object.type = '{!! trans('messages.prop.customer') !!}'--}}
                        } else if (object.type == 'shippings') {
                            {{--object.type = '{!! trans('messages.prop.shippings') !!}';--}}
                        }
            ...
            methods: {
                ...
                goToPage: function goToPage(val) {
                    ...
                    Vue.http.get('/api/v3/search/users/' + this.searchFilter, { limit: this.limit, start: ((this.start -1) * this.limit) }).then(function success(response) {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function (object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/users/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/users/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
