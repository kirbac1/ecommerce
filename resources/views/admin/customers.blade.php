...
@section('page.title', trans('messages.Customers'))
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    <tr v-for="(key, val) in localizedObjects | filterBy searchFilter" class="clickableRow">
                        ...
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.company }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.name }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.surname }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.vatid }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.type }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.phone }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.email1 }}; @{{ val.email2 }}</td>
                        <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.customer_group }}</td>
                       <!--  <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.orders.length }}</td> -->
        ...
        var vue = new Vue({
                ...
                localizedObjects: function() {
                    ...
                    this.objects.forEach(function(object) {
                        if (object.type == 'company') {
                            object.type = '{!! trans('messages.prop.company') !!}';
                        } else if (object.type == 'person') {
                            object.type = '{!! trans('messages.prop.person') !!}';
            ...
            methods: {
                ...
                goToPage: function goToPage(val) {
                    ...
                    Vue.http.get('/api/v3/search/customers/' + this.searchFilter, { limit: this.limit, start: ((this.start -1) * this.limit) }).then(function success(response) {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function(object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/customers/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/customers/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
