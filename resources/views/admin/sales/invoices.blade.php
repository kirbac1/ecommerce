...
@section('page.title', trans('messages.Invoices'))
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <th>{{ trans('messages.Invoice ID') }}</th>
                ...
                <tbody>
                    <tr v-for="(key, val) in localizedObjects | filterBy searchFilter" class="clickableRow">
                        ...
                        <td v-on:click="gotoDetails(val.id)">@{{ val.customer.company }}</td>
        ...
        var vue = new Vue({
            ...
            methods: {
                ...
                goToPage: function goToPage(val) {
                    ...
                    Vue.http.get('/api/v3/search/invoices/' + this.searchFilter, { limit: this.limit, start: ((this.start -1) * this.limit) }).then(function success(response) {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function(object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/invoices/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/invoices/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
