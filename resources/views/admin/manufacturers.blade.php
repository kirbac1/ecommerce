...
@section('page.title', trans('messages.Manufacturers'))
        ...
        var vue = new Vue({
            ...
            methods: {
                ...
                goToPage: function goToPage(val) {
                    ...
                    Vue.http.get('/api/v3/search/manufacturers/' + this.searchFilter, { limit: this.limit, start: ((this.start -1) * this.limit) }).then(function success(response) {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function(object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/manufacturers/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/manufacturers/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
                        response.data.result.forEach(function(element) {
                            element.selected = false;
                        });
                        this.$set('objects', response.data.result);
