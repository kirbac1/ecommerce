...
@section('page.title', trans('messages.Customer Groups'))
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <th>{{ trans('messages.Discount Percent') }}</th>
                    <th>{{ trans('messages.Enabled') }}</th>
                    <th>{{ trans('messages.Creation Date') }}</th>
                    <th>{{ trans('messages.Update Date') }}</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="(key, val) in localizedObjects | filterBy searchFilter" class="clickableRow">
                    ...
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.discountPercent }}%</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.enabled }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.created_at }}</td>
                    <td v-on:click.prevent="gotoDetails(val.id)">@{{ val.updated_at }}</td>
                </tr>
                </tbody>
            </table>
            <!-- Pagination -->
        ...
        var vue = new Vue({
            ...
            methods: {
                ...
                goToPage: function goToPage(val) {
                    ...
                    Vue.http.get('/api/v3/search/customergroups/' + this.searchFilter, { limit: this.limit, start: ((this.start -1) * this.limit) }).then(function success(response) {
                ...
                destroy: function destroy(event) {
                    ...
                    swal({
                        ...
                        that.objects.forEach(function (object) {
                            if (object.selected) {
                                Vue.http.delete('/api/v3/customergroups/' + object.id);
                            }
                        });
                        swal({
                ...
                loadObjects: function loadObjects(query) {
                    if (!query) query = '';
                    Vue.http.get('/api/v3/search/customergroups/' + query, { limit: this.limit, start: this.start - 1 }).then(function success(response) {
