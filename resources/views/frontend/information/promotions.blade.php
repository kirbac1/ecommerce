...
<div class="carta-container">
    <div id="container" class="container j-container">
        <ul class="breadcrumb">
            <li><a href="/">Home</a></li>
            <li><a href="/promotions">{{ trans('messages.promotions') }}</a></li>
        </ul>
        <div class="row">
            <div id="content" class="col-sm-12">
                <h1 class="heading-title">{{ trans('messages.promotions') }}</h1>

                <p v-if="discounts.length === 0">{{ trans('messages.no_offers') }}</p>

                <div class="row main-products product-grid">
                    <div class="product-grid-item xs-100 sm-50 md-33 lg-25 xl-25" v-for="discount in discounts" v-if="discount.product">
                        <div class="product-thumb product-wrapper">
                            <div class="image">
                                <a href="/product?id=@{{discount.product.id}}">
                                    <img class="first-image" style="max-width: 275px; max-height: 275px"
                                         :src="discount.product.image ? '/catalog/' + discount.product.image : '/assets/img/no-image.png'"
                                         :title="discount.product.name" :alt="discount.product.name">
                                </a>
                            </div>
                            <div class="product-details">
                                <div class="caption">
                                    <h4 class="name"><a href="/product?id=@{{discount.product.id}}">@{{discount.product.name}}</a></h4>
                                    <p class="offer-label">@{{discount.name}} &mdash; -@{{ percentOff(discount) }}%</p>
                                    <p class="price">
                                        <span class="price-new">@{{ discountedPrice(discount) | currency '€' }}</span>
                                        <span class="price-old">@{{ grossPrice(discount.product) | currency '€' }}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
...
new Vue({
    ...
    methods: {
        percentOff: function(discount) { return CartaPricing.percentOff(discount); },
        grossPrice: function(product) { return CartaPricing.grossPrice(product); },
        discountedPrice: function(discount) { return CartaPricing.discountedPrice(discount); },
 ...
 this.add2CartDisabled = false;
    ...
    ready: function() {
        ...
        Vue.http.get('/api/v3/discounts').then(function success(response) {
            this.$set('discounts', response.data);
        }.bind(this), function error(response) {
            console.log('FAILURE', response);
        });
    }
